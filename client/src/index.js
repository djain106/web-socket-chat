        ...
        return (
            <div>
                {this.state.isLoggedIn ?
                    <div>
                        <div className="title">
                            <Text type="secondary" style={{ fontSize: "36px" }}>Socket Chat</Text>
                        </div>
                        <div className="bottom">
                            <div style={{ display: 'flex', flexDirection: 'column', paddingBottom: 50 }} >
                                {this.state.messages.map(message =>
                                    <Card key={message.msg}
                                        style={{ width: 300, margin: '16px 4px 0 4px', alignSelf: this.state.user === message.user ? 'flex-end' : 'flex-start' }}
                                        loading={false}>
                                        <Meta
                                            avatar={
